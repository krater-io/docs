---
sidebarTitle: "Manage Krater Node"
title: "Manage Krater Node"
---
**Krater Node** is a program written in TS that serves as an interface between Krater’s backend and the Miners running inside a Data Center. Krater Node runs as a service managed by [PM2](http://pm2.io). The documentation for PM2 can be found [here](https://pm2.io/docs/runtime/overview/). 
...
`pm2 ls` - lists all pm2 processes running

`pm2 restart {id}` - restarts the PM2 process with {id} obtained from pm2 ls.

`pm2 sav` - saves the current state of processes and will restore them after reboot.

`pm2 delete {id}` - will stop and delete the process from the list.
